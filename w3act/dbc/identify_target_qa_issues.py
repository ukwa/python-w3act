...
def try_command(command):
    ...
    try:
        ...
        logging.error("Command exception trying to execute: {" + command + "}")
        logging.error("Output: " + output)
        sys.exit(0)
    logging.info("Command output for: {" + command + "}: " )
    logging.info(output)
...
def main():
    ...
    parser.add_argument('-c', '--dbconn', dest='db_connection_info', type=str, default='', required=False,
    ...
    if my_file.is_file() and (datetime.date.fromtimestamp(os.path.getmtime(my_file)) == datetime.date.today()):
        logging.info(W3ACT_JSON_FILE + " exists and was generated today.")
    else:
        logging.info("Today's " + W3ACT_JSON_FILE + " doesn't exist - deriving csv files.")
        try_command('w3act get-csv ' + db_connection_info)
        logging.info("Converting csv to: " + W3ACT_JSON_FILE)
        try_command("w3act csv-to-json")

    logging.info("File converted. Analysing...")
    ...
    try:
        with open(W3ACT_JSON_FILE) as f:
            all = json.load(f)
    except IOError:
        logging.error("Unable to access today's w3act data file: " + W3ACT_JSON_FILE)
        sys.exit(0)
